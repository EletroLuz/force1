...
local function pulse()
    ...
    if current_waypoint then
        ...
        if distance < 1 then
            ...
            if current_time - last_movement_time > 8 and force_move_cooldown == 0 then -- Tempo parado antes de forcar movimento 10 segundos
                console.print("Player stuck, using force_move_raw")
                pathfinder.force_move_raw(current_waypoint)
                force_move_cooldown = current_time + 5 -- 10 segundos de cooldown
            elseif current_time > force_move_cooldown then
